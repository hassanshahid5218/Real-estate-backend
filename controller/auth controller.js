...
async function handlegoogle(req,res,next){
   try{
       ...
       if (!email) {
         return next(errorhandler(400, 'Email is required for Google sign-in.'));
       }
       ...
       const username = name ? name.split(' ').join('').toLowerCase() + Math.random().toString(36).slice(-4) : 'googleuser' + Math.random().toString(36).slice(-4);
